...
class Basket:
    ...
    def add(self, product, product_qty):
        ...
        product_price = product.regular_price * product_qty

        if product_id not in self.basket:
            # add new basket info to session data
            self.basket[product_id] = {"total_price": str(product_price), "qty": int(product_qty)}
        else:
    ...
    def __iter__(self):
        ...
        products_in_basket = Product.objects.prefetch_related("product_image").filter(pk__in=product_ids)
